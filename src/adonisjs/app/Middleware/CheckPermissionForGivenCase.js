    ...
    try {
      ...
      if (Object.keys(params).length === 0) {
	    ...
	    if (properties[0] == 'author') {
		    ...
		    const author = await Database.raw(sqlQuery, [logged_user, case_id])

		    if (author != null) { await next() } else return response.status(500).json('you are not owner of this case')
	    }
      ...
      if (properties[0] == 'contributor') {
        const logged_user = auth.user.id
		    ...
		    const contributor = await Database.raw(sqlQuery, [logged_user, case_id])

		    if (contributor != null) { await next() } else return response.status(500).json('you are not contributor of this case')
      }
    } catch (e) {
    	console.log('quebrou')
    	console.log(e)
