...
class CreateNewPermissionsSchema extends Schema {
  ...
  async down () {
    const contributor = await Role.findBy('slug', 'contributor')
    await contributor.delete()

    const read = await Permission.findBy('slug', 'read')
    await read.delete()

    const editor = await Role.findBy('slug', 'editor')
    await editor.delete()

    const edit = await Permission.findBy('slug', 'edit')
    await edit.delete()
  }
}
